    ...
    async getUserReservations(): Promise<Reservation[] | { currentReservations: Reservation[]; pastReservations: Reservation[]; currentCount: number; pastCount: number }> {
        ...
        if (currentUser) {
            try {
                ...
                console.error('Error fetching reservations with Firebase auth:', error);
                return [];
            }
        }
    ...
    async getCombinedReservations(isGuest: boolean = false): Promise<Reservation[]> {
        try {
            if (isGuest) {
                ...
                // Handle serverReservations which might be an object or array
                const serverReservationsArray = Array.isArray(serverReservations) 
                    ? serverReservations 
                    : serverReservations.currentReservations || [];
                
                // Combine and deduplicate by ID
                const combined = [...serverReservationsArray, ...localReservations];
                const uniqueReservations = combined.filter((reservation, index, self) => 
                    index === self.findIndex(r => r.id === reservation.id)
                );
